...
def load_pretrained_model_custom_proj(model_path, model_base, model_name, mm_projector, load_8bit=False, load_4bit=False, device_map="auto"):
    kwargs = {"device_map": device_map}
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            print(mm_projector)
            mm_projector_weights = torch.load(mm_projector, map_location='cpu')
            mm_projector_weights = {k: v.to(torch.float16) for k, v in mm_projector_weights.items()}
            model.load_state_dict(mm_projector_weights, strict=False)
...
def load_mixed_pretrained_model(model_path, model_base, model_name, vision_tower_path, mm_projector_type, mm_projector_path, load_8bit=False, load_4bit=False, device_map="auto"):
    ...
    vision_args.mm_projector_type = mm_projector_type
    vision_args.vision_tower = vision_tower_path
    vision_args.pretrain_mm_mlp_adapter = mm_projector_path
    ...
    if 'llava' in model_name.lower() or "qwen" in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            if "qwen" in model_name or "Qwen" in model_base:
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            print('Loading LLaVA from base model...')
            if "qwen-14b" in model_name.lower() or "qwen-14b" in model_base.lower():
                model = LlavaQWenForCausalLM.from_pretrained(
                    model_base,
                    cache_dir=None,
                    use_safetensors=True,
                )
            elif "qwen-7b" in model_name.lower() or "qwen-7b" in model_base.lower():
                model = LlavaQWenForCausalLM.from_pretrained(
                    model_base,
                    cache_dir=None,
