...
def eval_model(args, questions, start, end, ans_file):
    ...
    for i in tqdm(range(start, end), desc="{} ({} - {})".format(threading.current_thread().name, start, end)):
        ...
        image_fn = osp.join(args.image_folder, question['image_id'] + ".jpg")
        if not osp.isfile(image_fn):
            continue
        if 'ocr_tokens' not in question:
            continue
        ...
        prompt += "\nReference OCR token: " + ", ".join(question['ocr_tokens'])
        ...
        out_j_dict_lst.append(out_j_dict)
    json.dump(out_j_dict_lst, ans_fp, indent=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hint-file", type=str, default="tables/question.jsonl")
    ...
    # questions file
    j_dict = json.load(open(args.question_file))
    questions = j_dict['data']
    hint_j_dict_lst = json.load(open(args.hint_file))['data']
    hint_dict = {}
    for hint_j_dict in hint_j_dict_lst:
        image_id = hint_j_dict['image_id']
        hint_dict[image_id] = hint_j_dict
    for question in questions:
        image_id = question['image_id']
        if image_id in hint_dict:
            question['ocr_tokens'] = hint_dict[image_id]
