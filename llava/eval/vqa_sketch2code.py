...
def eval_model(args, questions, start, end, ans_file):
    ...
    for fn in questions:
        gt_fp = open(fn)
        gt_lines = gt_fp.readlines()
        answer = ["".join(gt_lines)]

        image_file = fn[:-3] + "png"

        image_fn = osp.join(args.image_folder, image_file)
        ...
        prompt = "<|im_start|>user\n" + "Picture 1:<img></img>\n" + "Please generate DSL for the skect on this image:\n" + "<|im_end|>\n" + "<|im_start|>assistant\n"
        inputs += prompt
        ...
        output_j_dict = {}
        output_j_dict['fn'] = fn
        output_j_dict['label'] = answer
        output_j_dict['prediction'] = output_text
        out_j_str = json.dumps(output_j_dict) + "\n"
...
if __name__ == "__main__":
    ...
    for fn in files:
        if fn.endswith("gui"):
            questions.append(osp.join(args.image_folder, fn))
